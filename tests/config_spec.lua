...
describe("Using the theme", function()
  ...
  it("it should set a global variable", function()
    assert.equals("oncedark", vim.g.colors_name)
  end)

  it("it should apply the theme's color palette", function()
    local output = vim.api.nvim_exec2("hi Normal", true)
  ...
  it("it should be able to link to other highlight groups", function()
    local output = vim.api.nvim_exec2("hi TestHighlightGroup2", true)
  ...
  it("it should be able to load plugins that we have enabled", function()
    local output = pcall(vim.api.nvim_exec2, "hi OpSidebarHeader", true)
    assert.equals(true, output)

    output = pcall(vim.api.nvim_exec2, "hi AerialClass", true)
    assert.equals(false, output)
  end)

  it("it should be able to load filetypes that we have enabled", function()
    local output = pcall(vim.api.nvim_exec2, "hi @variable.javascript", true)
    assert.equals(true, output)

    output = pcall(vim.api.nvim_exec2, "hi @function.ruby", true)
    assert.equals(false, output)
  end)

  it("it should allow a color palette to be extracted", function()
    local colors = require("oncedark.helpers").get_colors(vim.g.colors_name)
    assert.equals("#d55fde", colors.purple)
    assert.equals("oncedark", colors.name)
  end)
  ...
  it("it changes the theme when the background changes", function()
    ...
    vim.cmd([[colorscheme oncelight]])
    assert.equals("oncelight", vim.g.colors_name)
  end)
  ...
  it("it changes colors and highlight groups when the background changes", function()
    ...
    vim.cmd([[colorscheme oncelight]])
    output = vim.api.nvim_exec2("hi Normal", true)
  ...
  it("it allows for highlight groups to be unset", function()
  ...
  it("it creates the autocmds for inactive windows", function()
    local output = vim.api.nvim_exec2("autocmd Oncedark", true)

    assert.equals(true, string.find(output, "Oncedark") ~= nil)
  ...
  it("it namespaces highlight groups", function()
    local output = vim.api.nvim_get_hl(23, { name = "NamespacedHighlightGroup" })
