...
---@return nil
function M.cache()
  local cache = require("oncedark.lib.cache")
  local themes = require("oncedark.theme").themes
  local compiler = require("oncedark.lib.compile")
...
---Clean all the theme's files on the disk
---@return nil
function M.clean()
  local cache = require("oncedark.lib.cache")
  local themes = require("oncedark.theme").themes
...
---Reset the color scheme to the default values
---@return nil
function M.reset()
  --TODO: To be implemented
  require("oncedark.config").reset()
  require("oncedark.override").reset()
end
...
function M.setup(opts)
  ...
  if not config.config.caching or config.config.debug then
    return M.cache()
  end
...
function M.load()
  ...
  if vim.fn.has("nvim-0.9") == 0 then
    return require("oncedark.utils.deprecate").write(
