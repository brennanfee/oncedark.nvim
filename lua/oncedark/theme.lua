...
function M.colors(theme)
  return require("oncedark.themes." .. theme).palette
end
...
function M.load(theme)
  ...
  theme = require("oncedark.themes." .. theme)

  -- Apply user color overrides directly to the theme
  if config and config.colors then
    theme.palette =
      require("oncedark.lib.palette").override(config.colors, theme.palette, theme.meta)
  end
