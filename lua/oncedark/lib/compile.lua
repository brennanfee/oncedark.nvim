...
local function resolve_value(value, theme)
  if type(value) == "table" then
    if value[theme.meta.background] then
      return value[theme.meta.background]
    end
    if value[theme.meta.name] then
      return value[theme.meta.name]
    end
    return
  end
...
function M.compile(theme, opts)
  theme = require("oncedark.theme").load(theme or config.theme)
  local highlight_groups, custom_groups = require("oncedark.highlight").groups(theme)
...
vim.o.background = "%s"]],
  ...
  if config.config.options.terminal_colors then
    local terminal_colours = require("oncedark.highlights.terminal").groups(theme)
  ...
  -- Autocmds
  if
    config.config.options.highlight_inactive_windows or config.config.options.window_unfocused_color
  then
    table.insert(lines, "\n-- Autocmds\n")
    local autocmds = require("oncedark.highlights.autocmd").groups(theme)
